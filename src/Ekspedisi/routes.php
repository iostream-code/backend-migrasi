    ...
    $app->group('/ekspedisi', function ($mod) use ($auth, $driver, $admin, $suratJalan, $poSuratJalan, $ekspedisi, $config) {
        ...
        $mod->group('', function ($group) use ($auth, $driver, $admin, $suratJalan, $poSuratJalan, $ekspedisi) {
            ...
            $group->group('', function ($adminGroup) use ($admin, $suratJalan, $poSuratJalan, $ekspedisi) {
                ...
                $adminGroup->post('/admin/sj/{id}/serah-terima', [$suratJalan, 'uploadSerahTerima']);
                $adminGroup->post('/admin/sj/{id}/validasi', [$suratJalan, 'validasi']);

                // --- "SJ Tarik" utk Purchase Order (pur_t_surat_jalan, MILIK
                // modul Purchase backend-production -- lihat docblock
                // App\Ekspedisi\Support\PoSuratJalan utk alasan lengkap) ---
                // Rute literal ('approved-po'/'outstanding-po'/'po/{id}/ready')
                // WAJIB didaftarkan SEBELUM '/admin/sj-po/{id}' -- pola sama
                // dgn '/admin/sj/years' vs '/admin/sj/{id}' di atas.
