-- Seed whitelist admin/dispatcher untuk ekspedisi-apk-backend (tabel ekspedisi_m_admin_access).
-- Tanpa ini tidak ada seorang pun yang bisa login sebagai admin di ekspedisi-apk --
...
-- admin/dispatcher app ekspedisi ini. Harus sama persis dengan kolom
-- `username` di shared_m_users -- tambah baris lagi (dipisah koma) kalau
-- admin-nya lebih dari satu.
-- ============================================================
INSERT INTO ekspedisi_m_admin_access (user_id)
SELECT user_id FROM shared_m_users
WHERE username IN (
    'ITAI'
)
-- Nama tabel WAJIB di-qualify di sini -- tanpa itu `user_id` ambigu (ada di
-- ekspedisi_m_admin_access maupun di shared_m_users lewat SELECT di atas).
ON DUPLICATE KEY UPDATE ekspedisi_m_admin_access.user_id = ekspedisi_m_admin_access.user_id;
...
FROM ekspedisi_m_admin_access a
